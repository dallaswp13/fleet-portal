...
export default function M360StatusIndicator() {
  const [status,  setStatus]  = useState<Status>('checking')
  const [message, setMessage] = useState('')
  const [showTip, setShowTip] = useState(false)

  useEffect(() => {
    fetch('/api/maas360/test')
      .then(r => r.json())
      .then(d => { setStatus(d.ok ? 'ok' : 'error'); setMessage(d.message ?? '') })
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
      <button
        ...
        title={`M360 API: ${label[status]}`}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text2)' }}>M360</span>
        <span style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => { setStatus('checking'); setMessage('');
              fetch('/api/maas360/test').then(r => r.json())
                .then(d => { setStatus(d.ok ? 'ok' : 'error'); setMessage(d.message ?? '') })
                .catch(() => { setStatus('error'); setMessage('Could not reach API') })
            }}

...
interface Props {
  transactions: Record<string,unknown>[]; page: number; totalPages: number; totalCount: number
  search: string; sort: string; dir: boolean; vehicle: string
}

export default function TransactionsTable({ transactions, page, totalPages, totalCount, search, sort, dir, vehicle }: Props) {
  ...
  const nav = useCallback((overrides: Record<string, string> = {}) => {
    const base = { q: search, sort, dir: dir ? 'desc' : 'asc', vehicle, page: String(page) }
    const p    = new URLSearchParams({ ...base, ...overrides })
    ;['q','vehicle'].forEach(k => { if (!p.get(k)) p.delete(k) })
    startTransition(() => router.push(`${pathname}?${p.toString()}`))
  }, [search, sort, dir, vehicle, page, pathname, router])
  ...
  function statusBadge(s: string | null) {
    const v = String(s ?? '')
    return <span className={`badge ${v === 'COMPLETED' ? 'badge-green' : v === 'REFUNDED' ? 'badge-red' : 'badge-gray'}`} style={{ fontSize: 10 }}>{v || '—'}</span>
  }

  function fmtCurrency(v: unknown) {
    const n = parseFloat(String(v ?? '0').replace(/[^0-9.-]/g, ''))
    if (isNaN(n)) return '—'
    const color = n < 0 ? 'var(--red)' : undefined
    return <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color }}>{n < 0 ? '-' : ''}${Math.abs(n).toFixed(2)}</span>
  }
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'stretch' }}>
        ...
        <div className="search-wrap" style={{ width: 160 }}>
          ...
          <input value={localV} onChange={e => setLocalV(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && nav({ vehicle: localV, page: '0' })}
      ...
      <div className="card">
          ...
          <table>
            ...
            <tbody>
              {transactions.map((t, i) => {
                ...
                const fleetFromDevice = String(t.device_name ?? '').replace(/^\*/, '').replace(/^\d+/, '').split('-')[0].toUpperCase() || null
                return (
                <tr key={i} onClick={() => vNum ? openByNumber(vNum, fleetFromDevice) : undefined}
        ...
        {totalPages > 1 && (
          <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 12, color: 'var(--text3)' }}>{totalCount.toLocaleString()} transactions · page {page + 1} of {totalPages}</span>
